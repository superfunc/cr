...
static int CR_STATE g_AttribLocationPosition = 0, g_AttribLocationUV = 0,
                    g_AttribLocationColor = 0;
static unsigned int CR_STATE g_VboHandle = 0, g_VaoHandle = 0,
                             g_ElementsHandle = 0;
...
bool ImGui_ImplGlfwGL3_CreateFontsTexture() {
    ...
    io.Fonts->GetTexDataAsRGBA32(
        ...
        &height); // Load as RGBA 32-bits (75% of the memory is wasted, but
                  ...
                  // to save on GPU memory. Upload texture to graphics system
    GLint last_texture;
    glGetIntegerv(GL_TEXTURE_BINDING_2D, &last_texture);
...
void imui_frame_begin() {
    ...
    if (g_data->get_window_attrib_fn(g_data->window, GLFW_FOCUSED)) {
        if (io.WantSetMousePos) {
            g_data->set_cursor_pos_fn(
                g_data->window, (double)io.MousePos.x,
                (double)io.MousePos
                    .y); // Set mouse position if requested by io.WantMoveMouse
                         // flag (used when io.NavMovesTrue is enabled by user
            ...
            io.MousePos = ImVec2(
                (float)mouse_x,
                (float)
                    mouse_y); // Get mouse position in screen coordinates (set
                              // to -1,-1 if no mouse / on another screen, etc.)
        }
    }

    for (int i = 0; i < 3; i++) {
        io.MouseDown[i] =
            g_data->mousePressed[i] ||
            g_data->get_mouse_button_fn(g_data->window, i) !=
    ...
    g_data->set_input_mode_fn(g_data->window, GLFW_CURSOR,
